...
<div class="col-sm-6">
	...
	{	
        ...
        $idlt=$_REQUEST['idlt'];
        $count=$p->luachon("select COUNT(lichkham.id) as count from lichkham inner join lichtrinh on lichkham.idlt=lichtrinh.id where lichkham.idlt=$idlt ");
		$soluong=$p->luachon("select soluong from lichtrinh where id=$idlt ");
		if($tenbn!='' && $tenck!='')
		{		if($count < $soluong){
				if($p->themxoasua("INSERT INTO lichkham(idck,idbn,idbs,giokham,lichngaykham,phidichvu,trangthai,trangthaithanhtoan,idlt) VALUES ('$tenck' ,'$tenbn', '$tenbs', '$giokham', '$today','$phidichvu','$trangthai','$trangthaithanhtoan','$idlt')")==1)
				{	
					echo '<script >alert("Thêm lịch khám thành công!"); window.location="appointments.php";</script>';					
					}
				else
				{
					echo' Tạo không thành công';
				}
                // echo '<script >alert("chưa đủ số lượng khám, '.$count.' vui lòng chọn lịch khác!"); window.location="add-appointment.php";</script>';					

            }
            else{
                echo '<script >alert("Đã đủ số lượng khám, vui lòng chọn lịch khác!"); window.location="add-appointment.php";</script>';					
...
?>
            <div class="m-t-20 text-center">                              
                <input type="submit"  class="btn btn-primary submit-btn" data-dismiss="modal" id="them" name="them" value="Tạo lịch khám"/>
            </div>
