...
<?php
  ...
  // $pdf->SetFont(($lang == 'zh' ? 'cid0cs' : 'Helvetica'), 'I', 8);
           ...
           $sqlmain= "select *, chuyenkhoa.tenck, bacsi.tenbs,phieukhambenh.ketluandieutri, hoadonthuoc.trangthaithanhtoan,hoadonthuoc.id,benhnhan.tenbn, donthuoc.ngaylap,chitietdonthuoc.slthuoc,chitietdonthuoc.gia,chitietdonthuoc.idthuoc, thuoc.tenthuoc from hoadonthuoc inner join donthuoc on hoadonthuoc.iddt=donthuoc.id inner join chitietdonthuoc on donthuoc.id=chitietdonthuoc.iddt inner join thuoc on chitietdonthuoc.idthuoc=thuoc.id inner join bacsi on donthuoc.idbs=bacsi.id inner join chuyenkhoa on bacsi.idck=chuyenkhoa.id inner join benhnhan on hoadonthuoc.idbn=benhnhan.id inner join phieukhambenh on benhnhan.id= phieukhambenh.idbn where hoadonthuoc.iddt=$id AND donthuoc.idbs=bacsi.id AND hoadonthuoc.idbn=$idbn AND hoadonthuoc.ngaylap=donthuoc.ngaylap";
...
//$sqlmain="select * from chitietdonthuoc order by iddt='$id' desc";
            ...
            button:hover {
              ...
              opacity: 0.9;
                  ...
                  <span id="watermark" style="font: size 13px;"><b>PHÒNG KHÁM ĐA KHOA ĐP</b></span>
                      ...
                      <span style="display:inline-block;margin-right:60px;"><strong>Mã hóa đơn:0'.$idhd.' </strong></span> <br>
                      <span style="color:red;display:inline-block;margin-right:60px;"><strong>Trạng thái:'.$trangthaithanhtoan.' </strong></span>
                    ...
                    <div class="row" style="margin:20px;font-size:15px; text-align:center">
                     ...
                     <span  id="ProductName">Họ và tên bệnh nhân : '.$tenbn.'</span><br>
                            ...
                            <span id="CustomerCountry">Ngày kê đơn: '.$ngaylap.'</span><br>
                            <span id="CustomerCountry">Kết luận điều trị: '.$ketluandieutri.'</span><br>
                      ...
                      <div class="row" style="font-size:12px;">
                        <span id="CustomerName">Bác sĩ kê đơn : '.$tenbs.'</span><br>
                        <span id="AccountProject">Chuyên khoa: '.$tenck.'</span><br>
 </div>
 <div class="equalHW eq nomargin-nopadding title" style="font-size:12px;">
                          Đơn thuốc
                        </div>
                        <div class="row" style="font-size:12px;">
                        <span id="CustomerName">Số thuốc : '.$sothuoc.'</span><br>
    ...
    <tr class="titles">
      ...
      <th>Đơn vị</th>
                     ...
                     ';
          $sqlmain1="select *,thuoc.lieuluong, thuoc.donvitinh,benhnhan.tenbn,benhnhan.sdt,benhnhan.diachi, benhnhan.email, benhnhan.ngaysinh, donthuoc.ngaylap,chitietdonthuoc.slthuoc,chitietdonthuoc.gia,chitietdonthuoc.idthuoc, thuoc.tenthuoc from chitietdonthuoc inner join donthuoc on donthuoc.id=chitietdonthuoc.iddt inner join hoadonthuoc on donthuoc.id=hoadonthuoc.iddt inner join thuoc on chitietdonthuoc.idthuoc=thuoc.id inner join benhnhan on hoadonthuoc.idbn=benhnhan.id where donthuoc.id='$id' order by donthuoc.id asc";
    ...
    <tbody>
                        ...
                        <td><span id="">'.$dem++.'<span></span></span></td>
                          <td id="Product"><span id="ProuductName">'.$tenthuoc.'</span></td>
  ...
  $content .= '</table>
                    ...
                    <div class="row">
                      ...
                      <div class="right">
                      <span><strong>Bác sĩ kê đơn</strong></span> <br><br><br><br><br>
                      <span>'.$tenbs.'</span>
 ...
 $file_name = "hoadonthuoc_".$datetime.".pdf";
 ob_end_clean();
